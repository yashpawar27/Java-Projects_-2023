...
//	private Card[] cards;
	...
	public ElevensBoard() {
		...
		super(RANKS, SUITS, new Card[BOARD_SIZE], 9, new Deck(RANKS, SUITS, POINT_VALUES));
	...
	public boolean anotherPlayIsPossible() {
		...
		for(int a = 0; a < cards.length; a++)
		{
			for(int b = 0; b < cards.length; b++)
			{
				...
				{
					System.out.println("Can make 11");
					return true;
				}
			}
		}
		...
		for(int a = 0; a < cards.length; a++)
		{
			if(cards[a].rank().equals("jack"))
			{
				j = true;
			}
			if(cards[a].rank().equals("queen"))
			{
				q = true;
			}
			if(cards[a].rank().equals("king"))
			{
				k = true;
			}
	...
	private boolean containsPairSum11(List<Integer> selectedCards) {
		...
		for(int a = 0; a < selectedCards.size(); a++)
		{
			...
			{
				if(cards[selectedCards.get(a)].pointValue() + 
						cards[selectedCards.get(b)].pointValue() == 11)
				{
					return true;
				}
			}
